...
(async function () {
  ...
  await exec('docker stop project-server');
  await exec('docker rm project-server');
  await exec('docker rmi project-server');

  console.log('Building new image');
  const buildResult = await exec('docker build --rm -t project-server .');

  if (buildResult) {
    console.log('Packing new image');
    fs.mkdirSync(outputDir);
    // await exec('docker save -o .\\dist\\project-server-%date:~0,4%%date:~5,2%%date:~8,2%%time:~0,2%%time:~3,2%%time:~6,2%.tar redis project-server');
    await exec('docker save -o .\\dist\\project-server.tar redis project-server');
    await copyFile('./model/config-production.json', './dist/shared/model-config.json');
    await copyFile('./bin/start.sh', './dist/start.sh');
  }
})()
  .then(res => {
